    ...
    <div class="container-fluid">
        ...
        <div class="row page-titles">
            <div class="col-md-5 col-8 align-self-center">
                <h3 class="text-themecolor m-b-0 m-t-0">Detail Tugas > {{$qz->user->full_name}}</h3>
            </div>
        </div>
            ...
            <div class="col-lg-8 col-md-8">
                <div class="card">
                    <div  style="text-align: center; margin-bottom: 8px;" class="card-header">
                        Kursus {{$qz->quiz->video->coursepackage->title}}
                    </div>
                    <div class="card-body">
                        <h4>Deskripsi Tugas <Tahap></Tahap></h4>
                        <p>{!! nl2br(e($qz->quiz->description)) !!}</p>
                        <h4>File</h4>
                        @if($qz->file == null)
                            <p style="color: red;">Belum Kumpul</p>
                        @else
                            <p>{{$qz->file}}&nbsp;&nbsp;<a href="{{route('download.file.othertask' , ['id'=>$qz->id])}}"><button class="btn-sm btn-info">Download</button></a></p>
                        @endif

                        <h4>Status</h4>
                        <form action="{{route('othertask.status.change' , ['id'=>$qz->id])}}" method="get">
                            {{csrf_field()}}
                            {{method_field('PUT')}}
                            <select id="status" name="status">
                                <option @if($qz->status == 1) selected @endif  value="1">Lolos</option>
                                <option @if($qz->status == 0) selected @endif value="0">Belum Lolos</option>
                            </select>

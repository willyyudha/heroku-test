...
@section('search-bar')
    <li class="nav-item hidden-sm-down search-box"> <a class="nav-link hidden-sm-down text-muted waves-effect waves-dark" href="javascript:void(0)"><i class="ti-search"></i></a>
        <form class="app-search" action="{{route('page.search.coursepackage.superadmin')}}" method="get">
            {{csrf_field()}}
                            ...
                            <table id="myTable" class="table table-bordered table-striped">
                                ...
                                @forelse($c as $d => $i)
                                    <tr>
                                        <th>{{$d+1}}</th>
                                        <th>{{$i->title}}</th>
                                        <th>{{number_format($i->price)}}</th>
                                        <th>{{$i->month_expired}} bulan</th>
                                        <th>{{$i->channel->title}}</th>
                                        <th>{{str_limit($i->description , 30)}}</th>
                                        <th>{{str_limit($i->preview_link , 30)}}</th>
                                        <th><button class="btn btn-info"><a style="color: white" href="{{route('packagecourse.show' , ['id'=>$i->id])}}">Ubah</a></button></th>
                                        <th>
                                            <form action="{{route('packagecourse.destroy' , ['id'=>$i->id])}}" method="post">
                                                {{csrf_field()}}
                                                {{method_field('DELETE')}}

            ...
            @section('search-bar')
                <li class="nav-item hidden-sm-down search-box"> <a class="nav-link hidden-sm-down text-muted waves-effect waves-dark" href="javascript:void(0)"><i class="ti-search"></i></a>
                    <form class="app-search" action="{{route('page.search.category.superadmin')}}" method="get">
                        {{csrf_field()}}
                                        ...
                                        <table id="myTable" class="table table-bordered table-striped">
                                            ...
                                            @foreach($category as $c => $i)
                                                <tr>
                                                    <th>{{$c+1}}</th>
                                                    <th>{{$i->title}}</th>
                                                    <th><button class="btn btn-info"><a style="color: white" href="{{route('category.edit', [$i])}}">Ubah</a></button></th>
                                                    <th>
                                                        <form action="{{route('category.destroy', ['channel' => $i -> id])}}" method="post">
                                                              {{csrf_field()}}
                                                              {{method_field('DELETE')}}

...
@section('search-bar')
    <li class="nav-item hidden-sm-down search-box"> <a class="nav-link hidden-sm-down text-muted waves-effect waves-dark" href="javascript:void(0)"><i class="ti-search"></i></a>
        <form class="app-search" action="{{route('page.report.video.search.all')}}" method="get">
            {{csrf_field()}}
    ...
    <div class="container-fluid">
        ...
        <div class="row page-titles">
            <div class="col-md-5 col-8 align-self-center">
                <h3 class="text-themecolor m-b-0 m-t-0">Data Report Video</h3>
            </div>
        </div>
        <!-- ============================================================== -->
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table id="myTable" class="table table-bordered table-striped">
                                ...
                                <tbody>
                                @forelse($report_video as $t => $r)
                                    <tr>
                                        <th>{{$t+1}}</th>
                                        <th>{{$r->user->full_name}}</th>
                                        <th>{{$r->video->name}}</th>
                                        <th>{{str_limit($r->reason,15)}}</th>
                                        <th>{{$r->video->coursepackage->title}}</th>
                                        {{--<th>{{$r->discussion->categories->title}}</th>--}}
                                        <th>
                                            <a class="btn btn-info" href="{{route('page.report.video.detail',['id'=>$r->id])}}">Detail</a>
                                        </th>
                                    </tr>
                                @empty
